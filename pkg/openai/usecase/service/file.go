...
func (s *fileService) UploadFile(ctx context.Context, req *ctrsvc.UploadFileRequest) (*ctrsvc.UploadFileResponse, error) {
	...
	if err != nil {
		return result, fmt.Errorf("[service.UploadFile] failed to resolve purpose: %w", err)
	}

	resp, err := s.client.RequestMultipart(ctx, "/files", map[string]string{"file": req.FilePath, "purpose": prp.Code})
	if err != nil {
		return result, fmt.Errorf("[service.UploadFile] failed to send multipart request: %w", err)
	}

	var file ctrsvc.File
	err = json.Unmarshal(resp, &file)
	if err != nil {
		return result, fmt.Errorf("[service.UploadFile] failed to unmarshal response: %w", err)
	}
...
func (s *fileService) GetFileInfo(ctx context.Context, req *ctrsvc.GetFileInfoRequest) (*ctrsvc.GetFileInfoResponse, error) {
	result := &ctrsvc.GetFileInfoResponse{}

	resp, err := s.client.Request(ctx, "GET", "/files/"+req.FileID)
	if err != nil {
		return result, fmt.Errorf("[service.GetFileInfo] failed to send request: %w", err)
	}

	var file ctrsvc.File
	err = json.Unmarshal(resp, &file)
	if err != nil {
		return result, fmt.Errorf("[service.GetFileInfo] failed to unmarshal response: %w", err)
	}
...
func (s *fileService) ListFiles(ctx context.Context, req *ctrsvc.ListFilesRequest) (*ctrsvc.ListFilesResponse, error) {
	...
	if err != nil {
		return result, fmt.Errorf("[service.ListFiles] failed to send request: %w", err)
	}
	...
	if err != nil {
		return result, fmt.Errorf("[service.ListFiles] failed to unmarshal response: %w", err)
	}
...
func (s *fileService) DeleteFile(ctx context.Context, req *ctrsvc.DeleteFileRequest) (*ctrsvc.DeleteFileResponse, error) {
	...
	if err != nil {
		return result, fmt.Errorf("[service.DeleteFile] failed to send request: %w", err)
	}

	err = json.Unmarshal(resp, result)
	if err != nil {
		return result, fmt.Errorf("[service.DeleteFile] failed to unmarshal response: %w", err)
	}

	if !result.Deleted {
		return result, fmt.Errorf("[service.DeleteFile] failed to delete file '%v'", result.ID)
	}
	return result, nil
}
